...
public class LastPage_map extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.lastpage_map);
	...
	class ShowAsynktask extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				String response = z_CustomHttpClient
						.executeHttpGet("http://hrm.testserver87.com/wedding/display_listing_web.php?listingid="
								+ listing_id);
		...
		protected void onPostExecute(Void result) {
			...
			try {
				...
				// Rect bounds = ratingBar.getProgressDrawable()
				// .getBounds();
				// ratingBar.setProgressDrawable(res.getDrawable(R.drawable.starsmall));
