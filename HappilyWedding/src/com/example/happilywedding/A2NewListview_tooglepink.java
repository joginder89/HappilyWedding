...
public class A2NewListview_tooglepink extends Activity {
...
static boolean flag_toggle=true;
	String str = null;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.secondpage_list_toogle_pink);
		...
		// TextView tvTime = (TextView) findViewById(R.id.textdatetime); try {
	 ...
	 * .getDefaultSharedPreferences(this);
	 * 
	 * String name = sharedPreferences.getString("DATE", "YourId");
	 * 
	 * return name;
	 * 
	 * }
	 ...
	 * .getDefaultSharedPreferences(this);
	 * 
	 * String name = sharedPreferences.getString("TIME", "YourTime");
	 * 
	 * return name;
	 * 
	 * }
	...
	class ShowAsynktask3 extends AsyncTask<Void, Integer, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			try {
				...
				String response = z_CustomHttpClient.executeHttpGet(url
						+ usrName);
		...
		protected void onPostExecute(Void result) {
			...
			try {
				if (arraylist.size() > 0) {
					ListViewAdapter_pink adapter = new ListViewAdapter_pink(
							A2NewListview_tooglepink.this, arraylist);
					...
					Toast.makeText(getApplicationContext(), "Internet Problem",
							3000).show();
				}
	...
	protected void onResume() {
		// TODO Auto-generated method stub
		super.onResume();
		IntentFilter intentFilter = new IntentFilter("android.toggleValue1");
		toggleValueReceiver = new BroadcastReceiver() {

			@Override
			public void onReceive(Context context, Intent intent) {
				if (intent.getAction().equalsIgnoreCase("android.toggleValue1")) {
					Bundle bundle = intent.getExtras();
					int toogleValue = bundle.getInt("PROGRESS_VALUE1");
					togglStatus = bundle.getInt("TOGGLE_STATUS1");
					// Toast.makeText(getApplicationContext(),
					// "Status" + togglStatus, 3000).show();
	...
	void sendServerValue() {
		new HttpAsyncTask()
				.execute("http://hrm.testserver87.com/wedding/updatetaskstatus.php?userid=3&taskid=3&statusid="
						+ togglStatus);
